...
print("MBIN node num is:",MBIN_nodes)
...
device = "cpu"#"cuda" if torch.cuda.is_available() else "cpu"4
...
def get_activation(name):
    def hook(model, input, output):
        ...
        MBON_value = activation['MBON_last']
        """
...
model2.layers[-1].register_forward_hook(get_activation('MBON_last'))
...
mem_py=mem_feature.numpy()
MBON_feature= torch.load("mbon_feature")
MBON_feature=torch.stack(MBON_feature)
MBON_py=MBON_feature.numpy()
...
def m_modle1(kc):
    ...
    """
    for i in range(4):
        cor_result = mem_py.dot(zc_np[i])/ (np.linalg.norm(mem_py, axis=1) * np.linalg.norm(zc_np[i]))
        index=cor_result.argsort()[::-1]
        #print(np.sort(cor_result)[::-1])
        zc[i]= mem_feature[index[0]]
     """
    for i in range(4):
        cor_result = mem_py.dot(zc_np[i]) / (np.linalg.norm(mem_py, axis=1) * np.linalg.norm(zc_np[i]))
        index = cor_result.argsort()[::-1]
        max_lab=[0]*10
        for j in range(50):
            max_lab[L_table[index[j]].item()]+=1
        for k in range(50):
            if(L_table[index[k]].item()==max_lab.index(max(max_lab)) and T_table[index[k]]):
                zc[i]=mem_feature[index[j]]
...
def m_modle15(kc):
    ...
    for i in range(4):
        cor_result = mem_py.dot(zc_np[i]) / (np.linalg.norm(mem_py, axis=1) * np.linalg.norm(zc_np[i]))
        index = cor_result.argsort()[::-1]
        if (T_table[index[0].item()]):
            pass
        else:
            #print("!!!!")
            label_index = L_table[index[0].item()]
            while (1):
                index_new = randint(0, 24999)
                if (label_index == L_table[index_new].item() and T_table[index_new]):
                    zc[i] = mem_feature[index_new]
                    break
...
def m_modle16(kc):
    ...
    for i in range(4):
        ...
        if (T_table[index[0].item()]):
            ...
            label_index = L_table[index[0].item()]
            for k in range(50):
                if (L_table[index[k]].item() == label_index and T_table[index[k]]):
                    zc[i] = mem_feature[index[k]]
                    break
...
def m_modle17(kc):
    ...
    for i in range(4):
        cor_result = mem_py.dot(zc_np[i]) / (np.linalg.norm(mem_py, axis=1) * np.linalg.norm(zc_np[i]))
        index = cor_result.argsort()[::-1]
        zc[i] = (mem_feature[index[0].item()] + mem_feature[index[1].item()]) / 2
...
def m_modle18(kc):
    ...
    for i in range(4):
        ...
        temp_mem = 0
        for j in range(15):
            if(np.isnan(cor_result[index[j]])):
                pass
            else:
                temp += np.exp(cor_result[index[j]])
        for k in range(15):
            if (np.isnan(cor_result[index[k]])):
                pass
            else:
                temp2 = np.exp(cor_result[index[k]])
                temp_mem += (temp2 / temp) * mem_feature[index[k].item()]
        zc[i] = temp_mem
...
def m_modle19(kc):
    ...
    for i in range(4):
        max_lab = [0] * 10
        for j in range(50):
            max_lab[L_table[index[i][j]].item()] += 1
        for k in range(50):
            if (L_table[index[i][k]].item() == max_lab.index(max(max_lab)) and T_table[index[i][k]]):
                zc[i] = mem_feature[index[i][k]]
...
def m_modle20(MBON):
    ...
    loss_result = torch.cdist(MBON, MBON_feature.to(device), p=1)
    sort, index = torch.sort(loss_result)
    Force_T=[]
    for i in range(4):
        if (T_table[index[i][0].item()]):
            ...
            T_label=L_table[index[i][0].item()]
            Force_T.append([i,T_label])
