...
def train_one_epoch(epoch_index,Round_ct,mem_start_flag):
    running_loss = 0.
    mem_end_flag=False
    ...
    for i, data in enumerate(training_loader):
        ...
        """
        if(not mem_start_flag):
            kc[:, :MBIN_node]=kc[:, :MBIN_node]+torch.empty(MBIN_node).normal_(mean=0.,std=1.)
        ...
        if(mem_start_flag):
            ...
            if(Round_ct>23996):
                mem_end_flag=True
                model_path = 'model_{}_{}'.format(epoch_index, "last train")
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        model.train(True)
        avg_loss,stop_flag= train_one_epoch(epoch_number,Round_ct,mem_flag)
        ...
        last_loss=avg_vloss
        if(loss_range<0.05 and epoch_number>40):
            early_stop_count+=1
        else:
            early_stop_count= 0
