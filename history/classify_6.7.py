...
print(len(mem_feature))
mem_feature=mem_feature[:10000]
print(len(mem_feature))
...
def m_modle1(kc):
    zc = kc[:, :MBIN_nodes]
    zc_py=zc.numpy()
    temp= torch.cat([mem_feature], dim=0)
    cor_result = np.corrcoef(zc_py, mem_py)

    for i in range(4):
        sort, index = torch.sort(cor_result[i], descending=True)
        find_flag = 0
        for j in range(50):
            if (T_table[index[j].item()]):
                match = mem_feature[index[j].item()]
                find_flag = 1
                #print('find!!!')
                break
        if (find_flag):
